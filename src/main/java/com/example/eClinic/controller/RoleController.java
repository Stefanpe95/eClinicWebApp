...
public class RoleController {
	  ...
	  private @ResponseBody ResponseEntity<Boolean> addRole(@RequestBody RoleDto role) 
		...
		public ResponseEntity<List<Role>> getAllRoles() {
			...
			if(list.isEmpty()) return new ResponseEntity<List<Role>>(list, HttpStatus.NOT_FOUND);
		...
		public ResponseEntity<Role> getRolebyId(@PathVariable("id") Integer id) {
			...
			if(role.isEmpty()) return new ResponseEntity<Role>(role.get(), HttpStatus.NOT_FOUND);
			else return new ResponseEntity<Role>(role.get(), HttpStatus.OK);
		}
		...
		public ResponseEntity<Role> deleteDepartment(@PathVariable("id") Integer id) {
	  		...
	  		if(obj.isEmpty()) return new ResponseEntity<Role>(obj.get(),HttpStatus.NOT_FOUND);
	  		else {roleService.deleteRole(id);
